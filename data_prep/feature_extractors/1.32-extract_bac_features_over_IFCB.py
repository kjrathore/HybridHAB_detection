...
OUTPUT_DIR = Path(
    "datasets/GULF_OF_MAINE/processed_data/sentinel_3_L2_past3days/bac_features2"
)
...
def get_config() -> SHConfig:
    config = SHConfig()
    config.sh_client_id     = 'sh-5cd7a174-8ceb-4b81-bf56-2f7f69f72421' 
    config.sh_client_secret = '5CJwHZUBIgclQ3MfpQSjrik1fqjiMbtu'  
...
def load_ifcb(file_path: str) -> pd.DataFrame:
    ...
    df = df[df["datetime"].dt.year > 2023].reset_index(drop=True)
    ...
    for name, (lat, lon) in STATION_UPDATES.items():
        ...
        if len(uniq)>0:
            ...
            df.loc[mask, "location_id"] = name
            print("no uniq location id ",name, uniq)
