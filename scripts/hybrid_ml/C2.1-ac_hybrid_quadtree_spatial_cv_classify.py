...
What's new in this version — multi-resolution blocking comparison
-----------------------------------------------------------------
  - The single fixed grid size (even Moran-calibrated) is one global
    bandwidth. BLOCKING_CONFIGS now runs THREE spatial partitioning
    schemes back to back as separate comparison tracks:
      "grid"              - existing Moran-swept fixed-size grid
      "quadtree_adaptive" - recursive quadtree, splitting stops once a
                             cell's point count drops to
                             QUADTREE_TARGET_OBS (block size tracks
                             local sample density: fine blocks at dense
                             stations, coarse blocks in sparse water)
      "quadtree_random"   - recursive quadtree, splitting stops by an
                             independent coin flip at each eligible
                             node (block size is NOT tied to density —
                             genuinely random, multi-resolution blocks)
      "clustering"        - size-constrained KMeans on (x_m, y_m); groups
                             are balanced in POINT COUNT rather than
                             geographic extent, which none of the three
                             schemes above enforce
    All three still respect a metres-based floor (leakage-relevant: a
    block smaller than the satellite pixel / ROMS grid autocorrelation
    scale lets train/test share near-duplicate signal even inside
    "different" GroupKFold folds) and a max depth / max iteration cap.
  - Everything downstream of "produces a block_id column" (merge_sparse_
    blocks, verify_no_block_split, GroupKFold, plotting) is unchanged —
    blocking scheme is just a different way of filling that one column.
  - Every result row (global/species/fold-level) now carries a
    "blocking" column so all four schemes' leakage_ratio and F1 can be
    compared directly in global_comparison.csv.
  - Diagnostic output (plots, per-blocking CSVs) is namespaced under
    BASE_OUT/<blocking_name>/... so schemes never overwrite each other.
  - Each scheme also gets an "alexandrium_warning_groups_labels.png":
    transparent bounding-box per group, points colored bloom positive/
    negative for Alexandrium catenella at the warning threshold, so
    group shape and label balance can be checked visually per scheme.
...
# Which blocking schemes to run, as separate comparison tracks. Comment any
# of these out to skip that scheme (e.g. drop "grid" once you trust the
# quadtree comparisons and just want the two adaptivity variants).
BLOCKING_CONFIGS = ["grid", "quadtree_adaptive", "quadtree_random", "clustering"]
...
# -- quadtree (both adaptive and random modes) --
QUADTREE_TARGET_OBS  = 30        # mode="adaptive": stop once a cell's point count drops to this
QUADTREE_STOP_PROB   = 0.3       # mode="random": P(stop) at each eligible node
QUADTREE_MIN_DEPTH   = 2         # both modes: force at least this many splits before allowing a stop
                                  # (mode="random" especially needs this -- otherwise a lucky early
                                  # coin flip can leave one block covering half the domain)
QUADTREE_MIN_BLOCK_M = 5_000     # both modes: leakage floor -- stop once a cell's shorter side hits this
QUADTREE_MAX_DEPTH   = 8         # both modes: hard cap regardless of mode-specific stop rule

# -- clustering (size-constrained KMeans -> roughly even point counts per group) --
CLUSTER_TARGET_OBS    = 30       # target points per cluster (n_clusters = n_valid / this)
CLUSTER_SIZE_MIN_FRAC = 0.7      # size_min = CLUSTER_TARGET_OBS * this
CLUSTER_SIZE_MAX_FRAC = 1.3      # size_max = CLUSTER_TARGET_OBS * this

MIN_BLOCK_OBS = 5                # drop (or merge) blocks with fewer obs than this
...
print("=" * 80)
print(f"HAB MULTI-LABEL CLF — BINARY RELEVANCE {MODEL_TYPE} (MULTI-SCHEME SPATIAL CV)")
print(f"Blocking schemes: {BLOCKING_CONFIGS}")
print("=" * 80)
...
BASE_OUT = Path(f"datasets/GULF_OF_MAINE/ml_outputs/rf_spatial_cv_{MODEL_TYPE.lower()}_multiblocking")


def out_dirs(blocking_name):
    """Per-blocking-scheme output roots, so schemes never overwrite each other."""
    root = BASE_OUT / blocking_name
    csv_dir  = root / "csv_results"
    imp_dir  = root / "importance"
    diag_dir = root / "diagnostics"
    for d in [csv_dir, imp_dir, diag_dir]:
        d.mkdir(parents=True, exist_ok=True)
    return csv_dir, imp_dir, diag_dir
...
def moran_sweep(df, target_col, candidate_radii_m, diag_dir, x_col="x_m", y_col="y_m",
...
def assign_quadtree_blocks(df, x_col="x_m", y_col="y_m",
                            mode="adaptive",
                            target_obs_per_block=QUADTREE_TARGET_OBS,
                            stop_prob=QUADTREE_STOP_PROB,
                            min_depth=QUADTREE_MIN_DEPTH,
                            min_block_size_m=QUADTREE_MIN_BLOCK_M,
                            max_depth=QUADTREE_MAX_DEPTH,
                            random_state=RANDOM_STATE):
    """
    Recursively quarter the bounding box. Two stopping rules:

      mode="adaptive": stop once a cell holds <= target_obs_per_block points.
        Block extent tracks local sample density -- fine blocks at dense
        stations (MVCO, Harpswell), coarse blocks in sparse open water.

      mode="random": stop by an independent coin flip (P=stop_prob) at each
        eligible node, regardless of how many points the cell holds. Block
        extent is unrelated to density -- some dense-station cells stay
        large, some sparse cells get split fine, purely by chance.

    Both modes always respect min_block_size_m (floor, to keep blocks
    larger than the satellite-pixel / ROMS-grid autocorrelation scale --
    this is the leakage-relevant constraint, not a density concern) and
    max_depth (hard cap). min_depth forces every branch to split at least
    that many times before either stopping rule is allowed to fire, so
    mode="random" can't degenerate into one giant first-guess block.

    Returns df with a "block_id" column (quadrant path string, e.g.
    "q0011" -- same role as the grid version, so merge_sparse_blocks /
    verify_no_block_split / GroupKFold all work unchanged) and a stats
    dict (block-size min/median/max in metres, via each leaf's bounding-
    box diagonal) worth reporting instead of a single BLOCK_SIZE_M.
    """
    df = df.copy()
    x = df[x_col].values
    y = df[y_col].values
    n = len(df)
    block_ids = np.empty(n, dtype=object)
    leaf_extents = []  # (width_m, height_m, n_obs) per leaf, for reporting
    rng = np.random.RandomState(random_state)

    def should_stop(idx, width, height, depth):
        if depth < min_depth:
            return False
        if depth >= max_depth or min(width, height) <= min_block_size_m:
            return True
        if mode == "adaptive":
            return len(idx) <= target_obs_per_block
        elif mode == "random":
            return rng.random() < stop_prob
        else:
            raise ValueError(f"Unknown mode: {mode!r} (expected 'adaptive' or 'random')")

    def recurse(idx, x0, x1, y0, y1, depth, path):
        width, height = x1 - x0, y1 - y0
        if len(idx) == 0:
            return
        if should_stop(idx, width, height, depth):
            block_ids[idx] = path
            leaf_extents.append((width, height, len(idx)))
            return
        xm, ym = (x0 + x1) / 2, (y0 + y1) / 2
        quads = {
            "0": idx[(x[idx] < xm) & (y[idx] < ym)],
            "1": idx[(x[idx] < xm) & (y[idx] >= ym)],
            "2": idx[(x[idx] >= xm) & (y[idx] < ym)],
            "3": idx[(x[idx] >= xm) & (y[idx] >= ym)],
        }
        bounds = {
            "0": (x0, xm, y0, ym), "1": (x0, xm, ym, y1),
            "2": (xm, x1, y0, ym), "3": (xm, x1, ym, y1),
        }
        for key, sub_idx in quads.items():
            bx0, bx1, by0, by1 = bounds[key]
            recurse(sub_idx, bx0, bx1, by0, by1, depth + 1, path + key)

    all_idx = np.arange(n)
    # Pad the outer bound slightly so points exactly on x.max()/y.max() are
    # still captured by "< xm" / "< ym" comparisons during recursion.
    pad = 1.0
    recurse(all_idx, x.min() - pad, x.max() + pad, y.min() - pad, y.max() + pad, 0, "q")

    n_unassigned = (block_ids == None).sum()  # noqa: E711
    if n_unassigned:
        raise RuntimeError(f"{n_unassigned} rows never reached a leaf — check recursion.")

    df["block_id"] = block_ids

    ext = np.array([(w, h) for w, h, _ in leaf_extents])
    diag = np.sqrt(ext[:, 0] ** 2 + ext[:, 1] ** 2)
    counts = np.array([c for _, _, c in leaf_extents])
    stats = {
        "mode":                mode,
        "n_blocks":            len(leaf_extents),
        "block_diag_min_m":    float(diag.min()),
        "block_diag_median_m": float(np.median(diag)),
        "block_diag_max_m":    float(diag.max()),
        "obs_per_block_min":   int(counts.min()),
        "obs_per_block_median": float(np.median(counts)),
        "obs_per_block_max":   int(counts.max()),
    }
    print(f"  Quadtree blocks ({mode}): {stats['n_blocks']:,}  |  "
          f"diag(m) min/median/max = {stats['block_diag_min_m']:.0f} / "
          f"{stats['block_diag_median_m']:.0f} / {stats['block_diag_max_m']:.0f}  |  "
          f"obs/block min/median/max = {stats['obs_per_block_min']} / "
          f"{stats['obs_per_block_median']:.0f} / {stats['obs_per_block_max']}")
    return df, stats


def assign_cluster_blocks(df, x_col="x_m", y_col="y_m",
                           target_obs_per_cluster=CLUSTER_TARGET_OBS,
                           min_frac=CLUSTER_SIZE_MIN_FRAC, max_frac=CLUSTER_SIZE_MAX_FRAC,
                           random_state=RANDOM_STATE):
    """
    KMeans-based spatial blocking with an explicit per-cluster size bound,
    so groups are balanced in POINT COUNT -- unlike grid blocks (fixed
    geographic size, point count varies with local sampling density) and
    unlike plain KMeans (geometrically compact, but point-count balance is
    only a side effect, not enforced). This directly answers "cells are
    probably not very balanced in terms of data points."

    Uses k-means-constrained (pip install k-means-constrained), which
    solves a size-constrained assignment problem rather than plain
    Lloyd's algorithm. Falls back to plain sklearn KMeans (no size
    guarantee) with a warning if that package isn't installed, same
    pattern as the libpysal/esda optional import above.
    """
    df = df.copy()
    coords = df[[x_col, y_col]].values
    n = len(df)
    n_clusters = max(1, round(n / target_obs_per_cluster))
    size_min = max(2, int(target_obs_per_cluster * min_frac))
    size_max = max(size_min + 1, int(target_obs_per_cluster * max_frac))
    # size_max * n_clusters must cover n, or the constrained solver is
    # infeasible -- relax size_max upward rather than failing.
    while size_max * n_clusters < n:
        size_max += 1

    try:
        from k_means_constrained import KMeansConstrained
        km = KMeansConstrained(n_clusters=n_clusters, size_min=size_min,
                                size_max=size_max, random_state=random_state, n_jobs=-1)
        labels = km.fit_predict(coords)
        mode = "clustering_constrained"
    except ImportError:
        print("  ⚠ k-means-constrained not installed — falling back to plain "
              "KMeans (cluster sizes NOT guaranteed balanced). Run "
              "`pip install k-means-constrained` for the size-balanced version.")
        from sklearn.cluster import KMeans
        km = KMeans(n_clusters=n_clusters, random_state=random_state, n_init=10)
        labels = km.fit_predict(coords)
        mode = "clustering_unconstrained"

    df["block_id"] = ["c" + str(l) for l in labels]
    ...
    for cid in np.unique(labels):
        ...
        ext.append(np.sqrt(w ** 2 + h ** 2))
    ext = np.array(ext)

    stats = {
        "mode": mode,
        "n_blocks": int(n_clusters),
        "block_diag_min_m": float(ext.min()),
        "block_diag_median_m": float(np.median(ext)),
        "block_diag_max_m": float(ext.max()),
        "obs_per_block_min": int(counts.min()),
        "obs_per_block_median": float(counts.median()),
        "obs_per_block_max": int(counts.max()),
    }
    print(f"  Clustering blocks ({mode}): {n_clusters:,} clusters  |  "
          f"obs/cluster min/median/max = {stats['obs_per_block_min']} / "
          f"{stats['obs_per_block_median']:.0f} / {stats['obs_per_block_max']}  |  "
          f"diag(m) min/median/max = {stats['block_diag_min_m']:.0f} / "
          f"{stats['block_diag_median_m']:.0f} / {stats['block_diag_max_m']:.0f}")
    return df, stats
...
def plot_spatial_blocks(df, output_path, lat_col="latitude", lon_col="longitude",
                         block_col="block_id", figsize=(10, 8), title_suffix=""):
    ...
    ax.set_title(f"Spatial blocks{title_suffix} (n={len(unique_blocks):,}, {len(df):,} obs)")
    ax.set_aspect("equal")
    fig.tight_layout()
...
def plot_cv_fold_assignment(df, groups, output_path, n_splits=N_CV,
    ...
    sc = ax.scatter(df[lon_col], df[lat_col], c=fold_assignment, cmap="tab10",
                    s=8, alpha=0.7, edgecolors="none", rasterized=True)
    ax.set_xlabel("Longitude"); ax.set_ylabel("Latitude")
    ax.set_title(f"GroupKFold test-fold assignment, {n_splits} folds{title_suffix}")
...
def plot_blocking_groups_with_species(df, species_col, threshold, output_path,
                                       block_col="block_id",
                                       lat_col="latitude", lon_col="longitude",
                                       figsize=(10, 8), title_suffix=""):
    """
    One figure per blocking scheme: a semi-transparent bounding-box
    rectangle per group, drawn from the actual lat/lon extent of the
    points assigned to that block_id (not a theoretical grid cell --
    merged grid blocks, quadtree leaves, and KMeans clusters are none of
    them perfect rectangles), with points colored by bloom positive/
    negative at `threshold` for `species_col`.

    Purpose: eyeball two things per scheme at once -- (a) whether the
    groups look spatially sensible, and (b) whether positive-class points
    end up concentrated in a handful of groups, which would make spatial
    CV unstable for that species regardless of which blocking scheme
    produced the groups.
    """
    import matplotlib.patches as mpatches

    d = df.dropna(subset=[species_col, lat_col, lon_col, block_col]).copy()
    is_pos = (d[species_col] > threshold).astype(int)

    fig, ax = plt.subplots(figsize=figsize)

    for bid, g in d.groupby(block_col):
        lon0, lon1 = g[lon_col].min(), g[lon_col].max()
        lat0, lat1 = g[lat_col].min(), g[lat_col].max()
        # pad degenerate (single-point / collinear) boxes so they're visible
        pad_lon = max((lon1 - lon0) * 0.05, 0.01)
        pad_lat = max((lat1 - lat0) * 0.05, 0.01)
        rect = mpatches.Rectangle(
            (lon0 - pad_lon, lat0 - pad_lat),
            (lon1 - lon0) + 2 * pad_lon, (lat1 - lat0) + 2 * pad_lat,
            linewidth=0.8, edgecolor="grey", facecolor="grey", alpha=0.12, zorder=1,
        )
        ax.add_patch(rect)

    ax.scatter(d.loc[is_pos == 0, lon_col], d.loc[is_pos == 0, lat_col],
               c="#4C72B0", s=10, alpha=0.6, label="negative", zorder=2, edgecolors="none")
    ax.scatter(d.loc[is_pos == 1, lon_col], d.loc[is_pos == 1, lat_col],
               c="#C44E52", s=14, alpha=0.85, label="positive (bloom)", zorder=3, edgecolors="none")

    ax.set_xlabel("Longitude"); ax.set_ylabel("Latitude")
    ax.set_title(f"{species_col} bloom labels over spatial groups{title_suffix}\n"
                 f"(threshold={threshold}, n_groups={d[block_col].nunique():,})")
    ax.set_aspect("equal")
...
# ──────────────────────────────────────────────────────────────────────────────
# Load & filter (shared across all blocking schemes)
# ──────────────────────────────────────────────────────────────────────────────
...
# AC validity masks and common-observation filtering are blocking-independent
# (they only depend on which AC columns are populated), so compute once and
# reuse across blocking schemes rather than recomputing three times.
print("\n" + "=" * 60)
print("COMMON OBSERVATIONS (blocking-independent)")
print("=" * 60)
ac_valid_masks = {}
...
common_mask = pd.concat(ac_valid_masks.values(), axis=1).all(axis=1)
print(f"  Common rows (all ACs): {common_mask.sum():,}")

roms_ok = "roms_available" in df_raw.columns
roms_mask = (df_raw["roms_available"] == True) if roms_ok else pd.Series(True, index=df_raw.index)

# ──────────────────────────────────────────────────────────────────────────────
# Global accumulators across ALL blocking schemes
# ──────────────────────────────────────────────────────────────────────────────
global_rows  = []
species_rows = []
fold_rows    = []
blocking_summary_rows = []  # one row per scheme: n_blocks, block-size stats

# ──────────────────────────────────────────────────────────────────────────────
# Outer loop: blocking scheme
# ──────────────────────────────────────────────────────────────────────────────
for blocking_name in BLOCKING_CONFIGS:

    print("\n" + "#" * 80)
    print(f"# BLOCKING SCHEME: {blocking_name}")
    print("#" * 80)

    CSV_DIR, IMP_DIR, DIAG_DIR = out_dirs(blocking_name)

    df_b = df_raw.copy()

    if blocking_name == "grid":
        if ENABLE_MORAN_SWEEP:
            print("  Running Moran's I sweep to calibrate block size...")
            corr_range_m = moran_sweep(df_b, MORAN_TARGET_COL, MORAN_CANDIDATE_RADII_M, DIAG_DIR)
        ...
        df_b = assign_metric_blocks(df_b, block_size_m)
        merge_floor_m = block_size_m
        block_stats = {"mode": "grid", "block_diag_median_m": block_size_m}

    elif blocking_name == "quadtree_adaptive":
        df_b, block_stats = assign_quadtree_blocks(df_b, mode="adaptive")
        merge_floor_m = QUADTREE_MIN_BLOCK_M

    elif blocking_name == "quadtree_random":
        df_b, block_stats = assign_quadtree_blocks(df_b, mode="random")
        merge_floor_m = QUADTREE_MIN_BLOCK_M

    elif blocking_name == "clustering":
        df_b, block_stats = assign_cluster_blocks(df_b)
        # No fixed geographic size to merge against; use this scheme's own
        # median group extent as the neighbor-merge distance scale.
        merge_floor_m = block_stats["block_diag_median_m"]

    else:
        raise ValueError(f"Unknown blocking scheme: {blocking_name!r}")

    df_b = merge_sparse_blocks(df_b, block_size_m=merge_floor_m)
    n_raw_blocks = df_b["block_id"].nunique()
    print(f"  Spatial blocks (all rows, post-merge): {n_raw_blocks:,}")

    blocking_summary_rows.append({
        "blocking": blocking_name,
        "n_blocks_all_rows": n_raw_blocks,
        **{k: v for k, v in block_stats.items() if k != "mode"},
    })

    # Apply the blocking-independent common/ROMS filters to this scheme's
    # blocked frame.
    df_common      = df_b[common_mask].copy()
    df_common_roms = df_b[common_mask & roms_mask].copy()
    print(f"  Common rows (all ACs): {len(df_common):,}  |  "
          f"blocks: {df_common['block_id'].nunique():,}")
    print(f"  Common rows with ROMS: {len(df_common_roms):,}  |  "
          f"blocks: {df_common_roms['block_id'].nunique():,}")

    # Diagnostic plots — generated once per scheme on the full common+ROMS
    # dataset, since block structure doesn't depend on AC or tier.
    print("\n  Generating spatial diagnostic plots...")
    plot_spatial_blocks(df_common_roms, DIAG_DIR / "spatial_blocks_map.png",
                         title_suffix=f" ({blocking_name})")
    ...
    # Group boxes + Alexandrium bloom labels, for this blocking scheme.
    # Uses df_common (not the ROMS-filtered set) so the check reflects the
    # full common-AC sample, independent of ROMS coverage.
    plot_blocking_groups_with_species(
        df_common, "Alexandrium_catenella",
        SPECIES_THRESHOLDS["Alexandrium_catenella"]["warning"],
        DIAG_DIR / "alexandrium_warning_groups_labels.png",
        title_suffix=f" ({blocking_name})")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Main AC / track / tier loop, for this blocking scheme
    # ──────────────────────────────────────────────────────────────────────
    for ac_name, ac_prefix in AC_PREFIXES.items():
        ...
        print(f"\n{'='*70}")
        print(f"  [{blocking_name}] AC: {ac_name}  |  spectral: {len(spec_only)}  |  ROMS: {len(roms_feats)}")
        print(f"{'='*70}")
        ...
        for track_key, tcfg in tracks.items():
            ...
            for tier in THRESHOLD_TIERS:
                ...
                global_rows.append({
                    ...
                    "block_diag_median_m":   block_stats.get("block_diag_median_m", np.nan),
        ...
        print(f"\n  [{blocking_name}] {ac_name} complete")

    print(f"\n[{blocking_name}] complete")

# ──────────────────────────────────────────────────────────────────────────────
# Save CSVs (combined across all blocking schemes, plus a blocking-scheme
# summary table)
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 80)
...
blocking_summary_df = pd.DataFrame(blocking_summary_rows)
blocking_summary_df.to_csv(BASE_OUT / "blocking_scheme_summary.csv", index=False)
print(f"  blocking_scheme_summary.csv ({len(blocking_summary_df)} rows)")

global_df  = pd.DataFrame(global_rows)
species_df = pd.DataFrame(species_rows)
fold_df    = pd.DataFrame(fold_rows)

global_df = round_floats(global_df.sort_values(["blocking", "tier", "ac", "track"]).reset_index(drop=True))
global_df.to_csv(BASE_OUT / "global_comparison.csv", index=False)
print(f"  global_comparison.csv  ({len(global_df)} rows)")

species_df = round_floats(species_df.sort_values(
    ["blocking", "tier", "ac", "species", "track"]).reset_index(drop=True))
species_df.to_csv(BASE_OUT / "species_comparison.csv", index=False)
...
if not species_df.empty:
    ...
    ].sort_values(["blocking", "tier", "ac", "delta_f1"], ascending=[True, True, True, False])
    round_floats(delta_df).to_csv(BASE_OUT / "roms_delta_summary.csv", index=False)
    print(f"  roms_delta_summary.csv")

fold_df = round_floats(fold_df.sort_values(
    ["blocking", "tier", "ac", "species", "track", "fold"]).reset_index(drop=True))
fold_df.to_csv(BASE_OUT / "fold_level_results.csv", index=False)
print(f"  fold_level_results.csv ({len(fold_df)} rows)")

print("\n" + "=" * 80)
print("MACRO CV F1 SUMMARY BY BLOCKING SCHEME")
print("=" * 80)
if not global_df.empty:
    pivot = global_df.pivot_table(
        index=["blocking", "ac", "tier"], columns="track",
        values="cv_f1_macro_mean", aggfunc="first").round(3)
    print(pivot.to_string())

    print("\nLeakage ratio (spatial/random F1) by blocking scheme:")
    leak_pivot = global_df.pivot_table(
        index=["ac", "tier", "track"], columns="blocking",
        values="leakage_ratio", aggfunc="first").round(3)
    print(leak_pivot.to_string())

    n_gate_fail = (global_df["leakage_gate_pass"] == False).sum()
    if n_gate_fail:
        print(f"\n  ⚠ {n_gate_fail} (blocking, track, tier) combinations fell below the "
...
print(f"\nDONE ({MODEL_TYPE}) — run clf_plots.py to generate comparison figures "
      f"(pass 'blocking' as an additional facet).")